...
const SeriesPage = (props) => {
    ...
    return (
        <div>
            {seriesData && page > seriesData.count / 20 || page < 0 ? (<p>error</p>) : (

                <div>
                    {seriesData && seriesData.map((serie) => {
                            return (
                                <div className="pagecontainer" key={serie.id}>
                                    {/* <a className="pagecontent" href={`/pokemon/${pokemon, index + 1 + pageNumber * 20}`}>{pokemon.name}</a> */}
                                    <Link className="pagecontent" to={`/series/${serie.resourceURI.split('/')[6]}`}>{serie.title}</Link>
                                </div>
                            )
                        }
                    )
